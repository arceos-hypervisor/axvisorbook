...
Axvisor 的整个 xtask 系统构建在 `ostool` 库之上，这是一个专门为操作系统开发项目设计的构建工具库，提供了许多基础功能。
...
通过将状态集中管理，避免了在各个模块之间传递大量参数，简化了代码结构。`AppContext` 来自 `ostool` 库，提供了基础的上下文功能，而 `Context` 在此基础上扩展了 Axvisor 特定的需求。
