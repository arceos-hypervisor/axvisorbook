---
title: "x86_64"
sidebar_label: "x86_64"
---

# x86_64

本文档介绍如何在 QEMU x86_64 环境中启动和验证 AxVisor 项目。在 QEMU x86_64 环境中，AxVisor 支持同时启动运行多个客户机操作系统，以下将启动 NimbOS 客户机操作系统。
...
```bash
# 启动单个 NimbOS 客户机（默认）
./quick-start.sh qemu-x86_64 start --nimbos

# 或者简写（默认就是 nimbos）
./quick-start.sh qemu-x86_64 start
```
...
```bash
# 第一步：准备环境（下载镜像、配置文件）
./quick-start.sh qemu-x86_64 setup

# 第二步：启动系统
./quick-start.sh qemu-x86_64 run --nimbos
```
...
- **QEMU**：版本 8.0 或更高版本，以便完整模拟 x86_64 硬件平台。目前 CI 中使用的是 QEMU 10.1.x 版本
...
1. 复制 QEMU 配置文件：
    ```bash
    cp .github/workflows/qemu-x86_64.toml tmp/configs/qemu-x86_64-runtime.toml
    ```
...
2. 修改 QEMU 配置文件中的 rootfs 路径：
    ...
    # 更新配置文件中的路径
    sed -i 's|file=${workspaceFolder}/tmp/rootfs.img|file='"$ROOTFS_PATH"'|g' tmp/configs/qemu-x86_64-runtime.toml
    
    # 验证修改
    grep "rootfs.img" tmp/configs/qemu-x86_64-runtime.toml
    ```
...
- **解决方案**：
    ...
    # 验证配置文件中的路径
    cat tmp/configs/arceos-x86_64-qemu-smp1.toml | grep kernel_path
    cat tmp/configs/linux-x86_64-qemu-smp1.toml | grep kernel_path
...
- **解决方案**：
    ...
    # 增加 AxVisor 日志级别
    sed -i 's/log = "Info"/log = "Debug"/g' tmp/configs/qemu-x86_64.toml

    # 重新构建并启动，查看详细错误信息
    cargo xtask build
    cargo xtask qemu --build-config tmp/configs/qemu-x86_64.toml --qemu-config tmp/configs/qemu-x86_64-runtime.toml --vmconfigs tmp/configs/客户机配置文件.toml
    ```
