...
```bash
# 启动单个 ArceOS 客户机（默认）
./quick-start.sh qemu-aarch64 start --arceos

# 启动单个 Linux 客户机
./quick-start.sh qemu-aarch64 start --linux

# 启动多个客户机（ArceOS + Linux）
./quick-start.sh qemu-aarch64 start --multi
```

### 分步执行

如果需要分步执行，可以使用 `setup` 和 `run` 命令：

```bash
# 第一步：准备环境（下载镜像、配置文件）
./quick-start.sh qemu-aarch64 setup

# 第二步：启动系统
./quick-start.sh qemu-aarch64 run --arceos    # ArceOS
./quick-start.sh qemu-aarch64 run --linux     # Linux
./quick-start.sh qemu-aarch64 run --multi     # 多客户机
```
...
2. 修改客户机配置文件中的 `kernel_path`，使其指向我们下载的客户机镜像：
    ...
    ```bash
    # 修改 ArceOS 客户机配置
    sed -i 's|^kernel_path = .*|kernel_path = "../images/qemu_aarch64_arceos/qemu-aarch64"|g' tmp/configs/arceos-aarch64-qemu-smp1.toml
    sed -i 's|^image_location = "fs"|image_location = "memory"|g' tmp/configs/arceos-aarch64-qemu-smp1.toml

    # 修改 Linux 客户机配置
    sed -i 's|^kernel_path = .*|kernel_path = "../images/qemu_aarch64_linux/qemu-aarch64"|g' tmp/configs/linux-aarch64-qemu-smp1.toml
    sed -i 's|^image_location = "fs"|image_location = "memory"|g' tmp/configs/linux-aarch64-qemu-smp1.toml
...
3. 修改 QEMU 配置文件中的 rootfs 路径：
    ...
    # 更新配置文件中的路径
    sed -i 's|file=${workspaceFolder}/tmp/rootfs.img|file='"$ROOTFS_PATH"'|g' tmp/configs/qemu-aarch64-runtime.toml
    # 将 success_regex 改为空数组
    sed -i '/success_regex = \[/,/\]/c\success_regex = []' tmp/configs/qemu-aarch64-runtime.toml

    # 验证修改
    grep "rootfs.img" tmp/configs/qemu-aarch64-runtime.toml
    ```
    ...
    **注意**：
    ...
    ```bash
    # 移除 virtio-blk 设备配置
    sed -i '/"virtio-blk-device,drive=disk0"/d' tmp/configs/qemu-aarch64-runtime.toml
    sed -i '/id=disk0,if=none,format=raw/d' tmp/configs/qemu-aarch64-runtime.toml
    sed -i 's/"root=\/dev\/vda rw init=\/init",//g' tmp/configs/qemu-aarch64-runtime.toml
    ```
...
- **解决方案**：
    ...
    # 重新构建并启动，查看详细错误信息
    cargo xtask build
    cargo xtask qemu --build-config tmp/configs/qemu-aarch64.toml --qemu-config tmp/configs/qemu-aarch64-runtime.toml --vmconfigs tmp/configs/客户机配置文件.toml
    ```
