...
- **Rust 开发环境**：根据 Rust 官网介绍安装 Rust 开发环境，然后参考项目根目录的 [`rust-toolchain.toml`](rust-toolchain.toml) 文件安装指定的编译工具链
...
使用 `cargo xtask defconfig qemu-aarch64` 命令设置 QEMU AArch64 为默认构建配置。这个命令会将 [`configs/board/qemu-aarch64.toml`](configs/board/qemu-aarch64.toml) 复制为 `.build.toml`，作为默认的构建配置。
