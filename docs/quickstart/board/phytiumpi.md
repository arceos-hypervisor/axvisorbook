...
- **飞腾派开发板**：基于飞腾 E2000 系列处理器，支持硬件虚拟化扩展
- **串口线**：用于连接开发板和主机，进行调试和日志输出
- **以太网线**：用于网络通信和镜像传输
- **电源适配器**：提供稳定的电源供应（具体规格请参考开发板手册）
...
- **U-Boot 引导加载器**：支持飞腾 E2000 开发板的版本
...
AxVisor 项目从 [axvisor-guest 仓库](https://github.com/arceos-hypervisor/axvisor-guest) 下载预构建的客户机镜像。这些镜像已经过飞腾派平台的优化和测试，可以直接用于验证 AxVisor 的功能。

CI 流程中使用的镜像包括：

- **飞腾派 ArceOS**: `phytiumpi_arceos`
  - 描述：适用于飞腾派开发板的 ArceOS 操作系统
  - 特点：轻量级、实时响应、专为飞腾 E2000 硬件优化
  - 内存需求：低，适合资源受限的环境
  - SHA256: `94f1b78498391b4dd9ddf4b56553dfd0e83deec7c6e8fb30812784a0115c5de7`

- **飞腾派 Linux**: `phytiumpi_linux`
  - 描述：适用于飞腾派开发板的 Linux 操作系统
  - 特点：功能完整、支持丰富的应用生态、针对飞腾 E2000 硬件优化
...
开发板配置文件定义了 AxVisor 在飞腾 E2000 硬件平台上的基本运行参数。配置文件位于 [`configs/board/phytiumpi.toml`](configs/board/phytiumpi.toml)，我们直接使用这个配置文件，不需要改动。
...
```bash
# 修改 ArceOS 客户机配置
sed -i "s|kernel_path = \"/path/to/arceos_aarch64-dyn_smp1.bin\"|kernel_path = \"tmp/images/phytiumpi_arceos/arceos_aarch64-dyn_smp1.bin\"|g" tmp/configs/arceos-aarch64-e2000-smp1.toml

# 修改 Linux 客户机配置
sed -i "s|kernel_path = \"/path/to/Image\"|kernel_path = \"tmp/images/phytiumpi_linux/Image\"|g" tmp/configs/linux-aarch64-e2000-smp1.toml
```
...
# 设备树文件（需要根据实际硬件调整路径）
export BOARD_DTB="path/to/phytium-e2000.dtb"
...
这个命令会将 [`configs/board/phytiumpi.toml`](configs/board/phytiumpi.toml) 复制为 `.build.toml`，作为默认的构建配置。
...
构建过程会编译 AxVisor 及其所有依赖项，生成适合在飞腾 E2000 平台上运行的二进制文件。构建完成后，可以在 `target/aarch64-unknown-none-softfloat/debug/` 目录下找到生成的二进制文件。
...
修改 [`configs/board/phytiumpi.toml`](configs/board/phytiumpi.toml) 或我们复制的 `tmp/configs/phytiumpi.toml`：
...
注意飞腾 E2000 的 CPU ID 格式，确保使用有效的物理 CPU ID。
...
# 对于轻量级客户机（如 ArceOS）
memory_regions = [
    [0x20_2000_0000, 0x1000_0000, 0x7, 1], # 256MB 内存
]

# 对于资源密集型客户机（如 Linux）
memory_regions = [
  [0x20_4000_0000, 0x2000_0000, 0x7, 1], # 512MB 内存
]
```
...
1. **内存布局**：飞腾派使用特殊的内存布局，客户机内存地址从 `0x20_0000_0000` 开始，这是飞腾 E2000 处理器的特殊内存映射要求

2. **CPU 核心分配**：飞腾 E2000 的物理 CPU ID 使用特殊格式（如 0x00、0x100），需要根据实际硬件配置

3. **设备树兼容性**：确保使用与飞腾 E2000 硬件兼容的设备树文件，包含所有必要的外设定义

4. **中断处理**：飞腾派的中断控制器（GIC）配置需要特别注意，确保正确配置中断模式

5. **国产化特性**：飞腾 E2000 是国产处理器，可能有一些特殊的硬件特性和限制，需要参考官方文档进行配置

通过以上优化措施和注意事项，可以显著提高 AxVisor 在飞腾派平台上的运行性能和稳定性。

## 参考资源

- [飞腾派硬件手册](https://www.phytium.com.cn/)
- [E2000 处理器数据手册](https://www.phytium.com.cn/)
