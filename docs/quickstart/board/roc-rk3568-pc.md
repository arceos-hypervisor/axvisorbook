...
```bash
# 一步启动 ArceOS 客户机
./quick-start.sh roc-rk3568-pc start --serial /dev/ttyACM0 --arceos

# 一步启动 Linux 客户机
./quick-start.sh roc-rk3568-pc start --serial /dev/ttyACM0 --linux

# 一步启动多客户机（ArceOS + Linux）
./quick-start.sh roc-rk3568-pc start --serial /dev/ttyACM0 --multi
```
...
# 第二步：编辑 tmp/configs/roc-rk3568-pc-runtime.toml，设置正确的串口设备
# 例如：serial = "/dev/ttyACM0"
...
# 准备环境并指定自定义串口设备
./quick-start.sh roc-rk3568-pc setup --serial /dev/ttyUSB0

# 查看完整帮助
./quick-start.sh roc-rk3568-pc --help
```

**注意**：
- 默认波特率为 `1500000`
- 如需修改波特率或串口设备，请编辑 `tmp/configs/roc-rk3568-pc-runtime.toml` 文件
